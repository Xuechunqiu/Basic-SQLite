...
def main():
    ...
    #row
    row = '(<row>)?[0-9]*(<row/>)'
    rowRegex = re.compile(row, re.I)
    result = re.finditer(rowRegex, file)
    for r in result:
        ...
    
    #terms
    term = '(<subj>)?.*(<subj/>)'
    termRegex = re.compile(term, re.I)
    result = re.finditer(termRegex, file)
    for r in result:
        words = re.sub("[^0-9a-zA-Z_-]", " ", r).split()
        for t in words:
